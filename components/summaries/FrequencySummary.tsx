...
export default function FrequencySummary() {
  ...
  const frequencyItemsArrays = useMemo(() => {
    if (!allItems || allItems.length === 0) return;

    return LEVELS['frequency'].map((level, index) => {
      if (chosenCategory) {
        return allItems.filtered('frequency == $0 AND category.name == $1', level, chosenCategory);
      } else {
        return allItems.filtered('frequency == $0', level);
      }
    });
  ...
  return (
    ...
    >
      {frequencyItemsArrays.map((filteredItems, index) => {
        return (
          <View key={index}>
          <Text style={themedStyles.heading} variant="titleMedium">{LEVELS.frequency[index]}</Text>
